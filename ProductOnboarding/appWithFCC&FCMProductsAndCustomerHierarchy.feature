@sanity
Feature: Onboard a product onboarding application with 2 products FCC&FCM and hierarchy as Customer
  ...
  Scenario: View the no of APIs at Handoff stage of a product onboarding application
    ...
    Then party, party provision,party enrichment for Cash Management and party enrichment for Corporate Channels APIs should be visible
  ...
  Scenario: Reject a product onboarding application
    ...
    Scenario Outline: Create an application with 2 products FCC&FCM and enrichment details for Customer hierarchy 'Is your Parent a transactional entity?' question as No and selects 'Do you want to create a Party Group for your Parent Entity?' as Yes
      Given a product onboarding application which is at Input stage
      And a product onboarding web portal for OPMAKER role
      When staff user logs in with the role
      When user selects the new application
...
#    And Parent Party ID on view of application
  ...
  Scenario Outline: Create an application with 2 products FCC&FCM and enrichment details for Customer hierarchy 'Is your Parent a transactional entity?' question as No and selects 'Do you want to create a Party Group for your Parent Entity?' as No
    Given a product onboarding application which is at Input stage
    And a product onboarding web portal for OPMAKER role
    When staff user logs in with the role
    When user selects the new application
