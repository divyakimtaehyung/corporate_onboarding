@sanity
Feature: Onboard a product onboarding application with all 4 products withoutEnrichment and hierarchy as Customer Group
  ...
  Scenario: View the no of APIs at Handoff stage of a product onboarding application
    ...
    Then 2 party group, 1 party provision APIs should be visible
  ...
  Scenario Outline: Create an application with all 4 products and without enrichment details with customer hierarchy
    Given a product onboarding application which is at Input stage
    And a product onboarding web portal for OPMAKER role
    When staff user logs in with the role
    When user selects the new application
